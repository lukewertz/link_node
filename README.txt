...
The syntax of these links ("[node:NNNN]") is incompatible with the attached_node and InsertNode modules.
...
Once you have enabled the link node filter as described above, you (or any user
...
applied.  Since the Link Node filter outputs HTML with line breaks and other
things, you'll probably want this filter to be the last one applied to the output.
If, for instance, the HTML filter is run after the Link Node filter, then
